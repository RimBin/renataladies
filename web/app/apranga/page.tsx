...
const CATEGORIES = ['Visi', 'Tampres', 'Liemenėlės', 'Viršutiniai', 'Kelnės', 'Šortai']

export default function AprangaPage() {
  const { add } = useCart()
  ...
  const filteredItems =
    selectedCategory === 'Visi'
      ? APPAREL_ITEMS
      : APPAREL_ITEMS.filter((item) => item.category === selectedCategory)

  const handleAdd = (item: (typeof APPAREL_ITEMS)[number]) => {
    add(
      {
        id: item.id,
        title: item.title,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-8 items-end mb-12">
        <div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-rlText mb-0">
            Sporto <span className="gradient-text">apranga</span>
          </h1>
        </div>
        <div>
          <p className="text-lg text-neutral-600">
            Drabužiai, kurie kvėpuoja, neslysta ir išlaiko formą net intensyviausiose treniruotėse.
          </p>
        </div>
      </div>
